...
function _doPost(e) {
  ...
  // O relatório vai com o PDF em base64: um termo de duas ou três páginas
  // passa dos 900 mil. E medir com e.postData.contents.length é de graça,
  // enquanto JSON.stringify(b) refaz o texto inteiro só para contar.
  var limiteTamanho = (b.acao === 'enviarRelatorio') ? 8000000 : 900000;
  var tamanhoPedido = (e.postData && e.postData.contents) ? e.postData.contents.length : 0;
  if (tamanhoPedido > limiteTamanho) return _json({ ok: false, erro: 'grande' });
  ...
  switch (b.acao) {
    ...
    /* ---------- agenda, relatório e consulta de saídas ----------
       Ficam aqui embaixo do _autenticar de propósito: sem isso,
       quem tiver o link /exec lê a agenda da Whizz e dispara email
       pela conta dela.                                            */
    case 'agenda':
...
/**
 * O app chama isto a cada alteração no acervo, 1,2s depois de mexer em
 * qualquer item. A versão anterior criava uma aba "Backup_data_hora" a
 * cada chamada, com carimbo em minutos — o que enchia a planilha de abas
 * e, pior, fazia duas edições no mesmo minuto colidirem no nome da aba:
 * a segunda estourava, virava erro de servidor, e não salvava.
 * Agora é uma aba só, reescrita: a rede de proteção continua, sem os
 * efeitos colaterais.
 */
...
/**
 * A coluna json já guarda a saída inteira, campos da agenda inclusive.
 * Em vez de acrescentar colunas na planilha, tiramos daí o título e a
 * data da gravação. Nenhuma aba muda de formato, e saída antiga apenas
 * aparece sem gravação vinculada — que é o correto.
 */
function lerHistorico(limite) {
  ...
  for (var i = v.length - 1; i >= 0; i--) {
    ...
    var linha = { id: String(v[i][0]), criadaEm: String(v[i][1]), fechadaEm: String(v[i][2]),
...
/* ===================================================================
   AGENDA, RELATÓRIO POR EMAIL E CONSULTA DE SAÍDAS
   =================================================================== */
...
/**
 * O PDF é montado no navegador, com o mesmo layout de sempre, e chega
 * aqui em base64. Este trecho só remonta e anexa.
 */
...
/**
 * Quais séries estão presas a alguma saída ABERTA — é isto que faz o
 * equipamento ficar vermelho no painel.
 *
 * Antes, o app só sabia da saída aberta no próprio aparelho: item que
 * outra equipe tinha levado aparecia verde, e dava para levar o mesmo
 * equipamento duas vezes. Saída fechada não entra aqui, então encerrar
 * devolve os itens para "na casa" sozinho.
 */
...
/**
 * Rode no editor uma vez: força o pedido de autorização da Agenda e do
 * Gmail e mostra no log se as duas estão de pé. Não envia email nenhum.
 */
function conferirAgendaEEmail() {
  ...
  Logger.log('Equipamentos presos em saída aberta: %s', seriesEmSaidasAbertas().length);
}